...
uniform vec4 colFog;       // fog/distance color (can differ from ground)
uniform vec4 colTint;      // texture tint color (brown for dirt, white for snow, etc)
...
void main() {
    ...
    // Ground color: flat or textured with distance fog
    ...
    if (texEnabled != 0) {
        ...
        // Normalize texture luminance to a -1..+1 detail signal
        float lum = dot(texColor.rgb, vec3(0.299, 0.587, 0.114));
        float detail = (lum - 0.125) / 0.125;  // centered on avg brightness
        // Apply detail as variation on tint color (tint controls the floor look)
