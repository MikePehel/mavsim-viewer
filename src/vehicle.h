...
typedef struct {
    ...
    int   pos_tier;          // 1=home_position, 2=GPOS/LPOS, 3=grid-placed
    float pos_confidence;    // 0.0-1.0 position confidence
    bool  pos_estimated;     // true if grid-placed estimate
    float temporal_confidence; // takeoff detection confidence (0.0-1.0)
} vehicle_t;
