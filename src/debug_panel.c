...
void debug_panel_draw(const debug_panel_t *d, int screen_w, int screen_h,
                      ...
                      int total_trail_points, bool ortho_visible,
                      int ortho_mode)
{
    if (!d->visible) return;
    ...
    // Frametime section: title + value + graph + peak line
    // Render section: title + 5 lines
    // Memory section: title + 4 lines
    float total_h = 0;
    total_h += line_h * 2 + graph_h + line_h;          // FPS
    total_h += section_gap;
    total_h += line_h * 2 + graph_h + line_h;          // Frametime
    total_h += section_gap;
    total_h += line_h * 6;                               // Render
    total_h += section_gap;
    total_h += line_h * 5;                               // Memory
    total_h += margin * 2;
    ...
    snprintf(buf, sizeof(buf), "Ortho panel %s", ortho_visible ? "ON" : "OFF");
    DrawTextEx(font, buf, (Vector2){cx, cy}, fs, 0, ortho_visible ? text_col : dim_col);
    cy += line_h;

    static const char *ortho_names[] = {"Persp", "Top", "Bottom", "Front", "Back", "Left", "Right"};
    snprintf(buf, sizeof(buf), "Camera      %s", ortho_names[ortho_mode]);
    DrawTextEx(font, buf, (Vector2){cx, cy}, fs, 0, text_col);
    cy += line_h;

    int render_passes = 1 + (ortho_visible ? 3 : 0);
    snprintf(buf, sizeof(buf), "Passes      %d", render_passes);
    DrawTextEx(font, buf, (Vector2){cx, cy}, fs, 0, text_col);
    cy += line_h;
    ...
    // Ortho textures
    int ortho_mem_kb = ortho_visible ? (3 * 512 * 512 * 4 / 1024) : 0;
    snprintf(buf, sizeof(buf), "Ortho tex   %d KB", ortho_mem_kb);
    DrawTextEx(font, buf, (Vector2){cx, cy}, fs, 0, ortho_visible ? text_col : dim_col);
    cy += line_h;

    // Font textures
    snprintf(buf, sizeof(buf), "Font atlas  1024 KB");
    DrawTextEx(font, buf, (Vector2){cx, cy}, fs, 0, text_col);
    cy += line_h;

    int total_kb = trail_mem_kb + ortho_mem_kb + 1024 + 5000;  // +5MB raylib overhead
